...
const pages = collectPages(distDir) // Usar dist/ original para leitura
const manuscritosArticles = pages.filter(p => p.route.startsWith('/manuscritos/') && p.route !== '/manuscritos/')

console.log(`\nPáginas originais: ${pages.length}`)
console.log(`Artigos manuscritos originais: ${manuscritosArticles.length}`)
...
test('splitHtml separa head do body', () => {
  ...
  assert(parts.head.includes('<title>T</title>'), 'head contém title')
  assert(parts.body.includes('<div>Body</div>'), 'body contém div')
  assert(!parts.body.includes('<head>'), 'body não contém <head>')
})

// Test 2
test('diffGroup com manuscritos encontra blocos comuns', () => {
  assert(manuscritosArticles.length >= 2, `Precisa de ≥2 artigos, tem ${manuscritosArticles.length}`)
  const result = diffGroup(manuscritosArticles)
  assert(result.blockCount > 0, `blockCount > 0 (got ${result.blockCount})`)
  assert(result.payloads.size === manuscritosArticles.length, `payloads = ${result.payloads.size}`)
})
...
// Test 6
test('cada payload tem head não-vazio', () => {
  const result = diffGroup(manuscritosArticles)
  for (const [route, payload] of result.payloads) {
    assert(payload.head && payload.head.length > 0, `${route} head vazio`)
    assert(payload.head.includes('<title>') || payload.head.includes('<meta'), `${route} head sem tags`)
  }
})
...
test('payload é menor que HTML original', () => {
  const result = diffGroup(manuscritosArticles)
  for (const page of manuscritosArticles) {
...
test('soma dos blocos não-vazios ≈ tamanho do body', () => {
  ...
  for (const page of manuscritosArticles) {
    ...
    const nonEmptySize = Object.entries(payload)
      .filter(([k, v]) => k !== 'head' && v && v.length > 0)
      .reduce((sum, [, v]) => sum + v.length, 0)
    const bodySize = splitHtml(page.html).body.length
...
test('páginas diferentes geram blocos diferentes', () => {
  ...
  const articlePage = manuscritosArticles[0]
  ...
  let hasDiff = false
  for (let i = 0; i < result.blockCount; i++) {
    if ((p1[String(i)] || '') !== (p2[String(i)] || '')) { hasDiff = true; break }
  }
...
test('INTEGRAÇÃO: processar no temp SEM modificar dist/', () => {
  ...
  // Rodar diff no temp
  const manTemp = tmpPages.filter(p => p.route.startsWith('/manuscritos/') && p.route !== '/manuscritos/')
